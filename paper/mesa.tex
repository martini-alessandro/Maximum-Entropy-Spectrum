...
\begin{document}
	...
	Stefano~{Schmidt}\inst{4,5}\thanks{s.schmidt@uu.nl} \and Gregory~{Ashton}\inst{6} \and Walter~{Del Pozzo}\inst{1}\thanks{walter.delpozzo@unipi.it}}
...
Let us start with the `information' (or information entropy): it is a measure of the degree of uncertainty on the outcomes of some experiment and specifies the length of the message necessary to provide a full description of the system under study. No information is brought if we are studying a system whose outcome is certain (the outcome is known with probability $p = 1$), as in this case, a communication is not even needed.
...
In principle, if the autocorrelation was known exactly (i.e. at every time $\tau \in (-\infty,+\infty)$), the computation of the PSD would reduce to a single Fourier transform (i.e. Eq.~\eqref{eq:psd-autocorrelation}). However, in any realistic setting, we are dealing with a finite number of samples $N$ from the process, hence such computation is impossible. Also, the single periodogram is not a consisten estimator for the power spectral density, since its variance doesn't decrease with the sample size N. 
...
The use of MESA is particularly useful for short time series samples (comment not clear to me), where Welch's method is outperformed in both precision and confidence.
...
\section{Computational Time for both MESA methods and Welch}
