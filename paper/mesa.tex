...
% comments command
\newcommand{\wdp}[1]{{\textcolor{green}{\texttt{SS: #1}} }}
...
In each of them, we will add more evindence (i.e. constraints on the optimization problem) and we will obtain different results.
...
%\end{align}
Thus MAXENT provide a deeper insight into the Gaussian distribution.
...
The computation of the power spectral density is an important component in stationary time series analysis and characterization.
In principle, if the autocorrelation was known exactly (i.e. at every time $\tau$), the computation of the PSD would be just a single Fourier transform.
However, in every realistic setting, we are dealing with a finite number of samples $N$ and it is not possible such precise computation.
