...
The coefficients $a_s (s > 0)$, together with an overall multiplicative scale factor $P_N$, are to be determined by an iterative process (called Burg's algorithm)\footnote{There are two implementations of Burg's algorithm available in the memspectrum package: 'Standard' and 'Fast.' 'Standard' is slower but more stable for solving Levinson recursion, while 'Fast' is quicker but less stable. On simulated stationary data, both versions typically yield similar results. However, when working with real gravitational waves data, we found that the 'Fast' implementation introduces noise into the PSD estimate without improving accuracy. Therefore, it's advisable to use the 'Standard' implementation whenever possible. Keep in mind that if instability arises, a message will appear, however, it doesn't guarantee identical results between the two configurations if the message is not displayed.}. \\
