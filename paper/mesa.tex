...
>>>>>>> b4656241deb9202eeb7a737395b29a26362866fe
...
In doing so, we generate $100$ autoregressive processes $AR(p)$ with a random value of $p$, drawn such that $log(p)~\mathcal{U}[log(2), log(5000)]$.
Each coefficient $a_k$ is assigned as follows: $a_k = \alpha_k e^{-0.05\cdot k}$, where $\alpha ~ \mathbb{N}(0,0.1)$ and the exponential ensures that the coefficients falls with distance (as in any realistic finite memory process).
