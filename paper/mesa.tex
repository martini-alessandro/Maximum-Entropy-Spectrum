...
Thus, by maximizing eq.~\ref{eq:EntropyGain} with constraints in eq.~\ref{eq:MaxConstraint}, we can give an estimate of the spectrum given an empirical time series.
...
\section{Application: Temperature Time Series} \label{sec:temp_timeseries}
...
We downloaded the historical reanalysis data from the Copernicus project (or whathever it is) \cite{}.
...
Higher order harmonics (corresponding to integer multiples of $f_D$) are also visible up to the Nyquist frequency ($f_D = \SI{0.5}{hour^{-1}}$): they are required to explain the high variability of the daily temperature cycle throughout the year.
...
This can be understood by looking at fig.~\ref{fig:p_vs_ptrue}. It is shown that CAT systematically select very long filters (i.e. large $p_{CAT}$ values for the autoregressive orders), whereas FPE tends to select smaller values for the order $p_{FPE}$ of the autoregressive process, often understimating badly the actual value.
...
On the other hand, the confidence interval is pretty large (almost as large as $\SI{15}{K}$), thus making easy for the actual data to fit the predictions. We believe that the origin of such large uncertainty is due to the complexity of the time series considered: any realistic model, even more complex than our autoregressive process, might not produce drastically lower uncertainties (in such a long term).
...
Unlike the standard approach, we do not enforce the constraints in eq.~\ref{eq:MaxConstraint} with the standard Lagrange Multipliers approach.
...
\end{equation}
and the entropy gain expression eq.~\ref{eq:EntropyGain} becomes:
\begin{equation}
    \Delta H = \int_{-Ny}^{Ny}  
    \log\left(\frac{1}{2 Ny}\sum_{n = -\infty}^{\infty} \bar r_n e^{-\imath 2 \pi f n \Delta t} 
    \right) df.
\end{equation}
Note that this expression already takes into account the contraints in eq.~\ref{eq:MaxConstraint}.
...
\begin{equation}
	...
	\lambda_s = \lambda_{-s}^*. 
\end{equation}
The second property is obtained considering that the autocorrelation function $r_n$ can only be computed for a finite time interval $n \in [-N, N]$ and that the PSD estimation must not depend on the unavailable values $r_n$: this is part of the constraint in eq.~\ref{eq:MaxConstraint}
...
From eq.~\ref{eq:lamdas} and from the properties above, is easily seen \sschmidt{How exactly?} that $S(f)$ can be expressed via a Fourier Series 
...
To compute the $a_s$ is convenient to plug into eq.~\ref{eq:MaxConstraint} the Laurent Polynomial exansion for $S(f)$ eq.~\ref{eq:MESApsd_appendix} and then integrating over $z$. In this way the equation becomes:
\begin{equation}
...
\section{Levinson recursion} \label{sec:MESA_solution}
The solution of the eqs.~(\ref{eq:errorFilter1}-\ref{eq:errorFilter2}) fully determines the functional form of the power spectral density estimator \ref{eq:MESApsd_appendix}.
...
The 0-th order element can be easily initialized reminding that $a_0 = 1$ (always) and that $P_0$ can be determined from \ref{eq:errorFilter1}.
...
These expressions allow us to compute $\vec a$ and $P_N$ to any order by simply iterating \ref{eq:Levinson1} and \ref{eq:Levinson2}. Substituing them in equation \ref{eq:MESApsd_appendix} the problem of the estimation for the power spectral density via maximum entropy principle is solved.
