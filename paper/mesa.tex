...
The last criterion we will consider is the Optimum Bayes Decision rule (OBD)\cite{doi:10.1029/WR018i004p01097}. With the technique of asymptotic integration, the loss function is proposed to be the length that maximizes the posterior distribution \sschmidt{Which posterior?}: 
   \begin{align}
...
\begin{equation}\label{eq:freq_erorr}
...
\begin{figure}
	...
	\caption{Synthetic spectrum (left) and relative autocorrelation function (right) used to simulate the data to assess the performance of the three loss functions considered in Sec.~\ref{sec:arp_validation}\sschmidt{This should be done with the same style as the others}.}
	\label{fig:autocorr}
\end{figure}
...
\begin{figure}
    ...
    \caption{Noise in both time and frequency domain. Frequency noise behaviour is representative of the chosen spectrum, having its maximum amplitude around $f = 2.5 au$ \sschmidt{Togliamo questa figura?}\wdp{direi di si}}
    \label{fig:noise}
\end{figure}
...
\begin{figure}
    ...
    \caption{Original power spectral density from Ligo \sschmidt{Remove this?? It's already in the other figure}}
    \label{fig:LigoPSDOriginal}
\end{figure}
In this subsection we concentrate again on the reconstruction of a specific, known, power spectrum: that is the Advanced LIGO design sensitivity theoretical spectral curve see fig.~\ref{fig:LigoPSDOriginal}.
...
\begin{figure}
    \centering
     \includegraphics[width = \linewidth]{Images/optimisers_comparison/ligo/compare_estimates.pdf}
      \caption{Average spectrum for the three different loss function as compared with the ``true" PSD. The average is computed with $1000$ realization of a $3000$ points long time series.}
...
In figure \ref{fig:ligo1peak} and \ref{fig:ligo2peak}, we plot a details od fig.~\ref{fig:ligospectrum} around the two peaks of the spectrum
In figure~\ref{fig:LigoOrderError}, we report, as above, a scatter plot with the frequency averaged error (eq.~\ref{eq:freq_erorr}) and the length of the autoregressive model obtained with each loss function.
...
In fact, even in presence of very sharp peaks, CAT reconstruction seems to be almost perfectly coincident with each of them (figs.\ref{fig:ligo1peak} and \ref{fig:ligo2peak}).
...
The proposed solution follows the original demonstration due to Burg \cite{burg1975maximum}. The constraints (\ref{eq:MaxConstraint}) can be directly implemented by substituting $S(f)$ as the Fourier transform of the autocorrelation function
\begin{equation}
    S(f) = \frac{1}{2 Ny}\sum_{n = -\infty}^{\infty} \bar r_n e^{- \imath 2 \pi n \Delta t}
\end{equation}
in equation \ref{eq:EntropyGain}. In this way the variational problem is reduced to the maximization of: 
\begin{equation}
    \int_{-Ny}^{Ny}  
    \log\left(\frac{1}{2 Ny}\sum_{n = -\infty}^{\infty} \bar r(n\Delta t) e^{-\imath 2 \pi f n \Delta t} 
    \right) df.
\end{equation}
Since the autocorrelation function can only be computed for a finite time interval $n \in [-N, N]$, the requirement for our estimate to be independent from the unkown values of the autocorrelation function can be implemented as:  
\begin{equation}\nonumber 
    \frac{\delta H}{\delta \bar r_s} = 0 \text{ for } \vert s \vert > N.
\end{equation}
By explicitly computing the derivative: 
\begin{equation}
      \frac{\delta H}{\delta \bar r_s} = \frac{1}{2Ny}\int_{-Ny}^{Ny} S(f)^{-1}e^{-\imath 2 \pi f s \Delta t } df = \lambda_s
\end{equation}
is easily seen that $S(f)$ can be expressed via a Fourier Series
\begin{equation}\label{eq:PSDconstraint}
    S(f)^{-1} = \sum_{s = -\infty}^\infty \lambda_s e^{-\imath 2 \pi f s \Delta t}
\end{equation}
with the properties $\lambda_s = 0$ for $\vert s \vert > N$ and $\lambda_s = \lambda_{-s}^*$.  
...
\end{equation}
It is easy to show that if $z_0$ is a root for the polynomial $(z_0^*)^{-1}$ is also a root: for every root laying outside the unit circle there will be another root inside of it and vice-versa. \\ 
...
%The MESA expression for the power spectral density admits a non-linear, all pole representation when evaluated in $z = e^{-\imath 2 \pi f \Delta t}$. All the roots of the first polynomial can be chosen to lay outside the unit circle, so that all the roots of the second polynomial will lay inside of it \cite{1975STIN...7714318B}. This is equivalent to (wide-sense) stationarity of the observed process. 
...
\end{equation}
From now on only wide-sense stationary processes will be considered: for such a process all poles lay outside the unit circle so that the previous integral can be easily computed obtaining the following results: 
...
\end{align}
The $N$ equations obtained for $r = 0$ can be solved via the Lenvoinson-Durbin recursion \cite{doi:10.1002/sapm1946251261} and are sufficient to uniquely determine prediction error filter $\vec a$ and $P_N$. Defining the quantities  
\begin{align}
...
The recursion can be solved for every order if the 0'th order elements are computed. Reminding that $a_0 = 1$ whatever the order the other coefficients are easily found to be: 
\begin{equation}
P_0 = R(0); \quad \Delta_0 = R(1); \quad c_0 = -\frac{R(1)}{R(0)}. 
\end{equation}
These expressions allow us to compute $\vec a$ and $P_N$ to whatever order by simply iterating \ref{eq:Levinson1} and \ref{eq:Levinson2}. Substituing them in equation \ref{eq:MESApsd_appendix} the problem of the estimation for the power spectral density via maximum entropy principle is solved.
\amartini{Io lascerei questo senza tutta la descrizione esplicita della Levinson recursion, che comunque lascio sotto se vogliamo tenerla} 
