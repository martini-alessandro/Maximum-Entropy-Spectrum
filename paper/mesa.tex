...
Thus maximising Eq.~\eqref{eq:EntropyGain} is equivalent to maximise Eq.~\eqref{eq:entropy_continuos}.
...
This approach on PSD computation provides a result consistent with the epirical autocorrelation function whenever this is available and, at the same time, it does not make any assumption for the unavailable estimates for the autocorelation at large time lags.
...
In a last subsection~\ref{sec:LIGO_validation}, the same analysis is performed on syntetic data generated from the analytical spectrum released by the LIGO-Virgo collaboration.
