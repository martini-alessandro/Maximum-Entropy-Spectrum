...
systematically lower variance. This is particularly manifest in the case of a little number of data points, making Burg's method most suitable to work in this regime.
...
Besides the window function, Welch' method requires a number of arbitrary choices to be made, since the number of time slices and the overlap between consecutive slices. All the knobs must be tuned by hand and their choice can dramatically affect the PSD estimation.
The final PSD estimate depends on all such choices, hence begging the question of what the ``real" PSD is.
...
Our goal is to bring (again) to public attention Maximum Entropy Spectral analysis, in the hope that it will be widely employed as a way out to the many problem posed by Welch's algorithm (or other similar methods).
...
We also compare our results with those of spectral analysis carried out with the standard Welch method.
...
This defines a variational problem, where a numerical measure of uncertainty $H\left[p_1, \dots, p_N\right]$ has to maximized. 
...
p&\left((\tilde{x}_1, \ldots, \tilde{x}_n)\vert I\right) = \nonumber \\
    &\frac{1}{\left(2 \pi \det S\right)^{k / 2}}\exp\left(-\frac{1}{2}\sum_{ij}\tilde{x}_i S^{-1}_{ij} {x}_j \right)\,,
\end{align}
...
not on the PSD.
It can be proved  \cite{AblesMESA, Bartlett} that the variational problem can be formulated in terms of the power spectral density $S(f)$ alone by considering our signal as the result of filtering a white noise process with a filter with transfer function $S(f)$\footnote{
...
\sschmidt{Make a good plot here...}
...
We can also note that MESA converges to the real spectrum faster than Welch method, providing a better estimate expecially for short time series. This behaviour is more evident if the low frequencies are considered. An accurate profile reconstruction can be obtained with MESA using a 5 seconds-strain only, while Welch method requires at least 10 seconds of data to obtain a comparable profile.
This is true in the low frequency sector, where the estimate requires longer time windows.
Another important element is the estimation noise: we find that the PSD estimation provided by the Welch's method is more noisy (i.e. has a large number of spurious peaks) rather than PSD provided by MESA with FPE loss function. If longer datasets are considered, even if MESA is found to be a little more accurate and less noisy than Welch, all of the above differences become negligible. 

As already discussed Welch, method is very instable with respect to the windowing choices we made. We tried to find the best compromize between noise and accuracy for the reconstruction, but different choices can be made, possibly providing more accurate results. We want to stress that this fact doesn't invalidate our discussion: one of the most appealing advantages of MESA is the absence of fine tuning.
A more accurate reconstruction with Welch can only be obtained introducing a greater amount of noise: a greater accuracy requires longer sub-segments of data, reducing the number of spectra over which we average. Reducing noise require the opposite choices, and we can conclude that MESA represents a better compromise between noise and accuracy than Welch's method.  
...
\end{equation}
where $\tilde{\hphantom{a}}$ denotes Fourier transform and $x_t(\theta)$ is a prediction for the time series dependent on some parameter $\theta$.
This likelihood corresponds to looking for a signal $x_t$ into noisy data.
...
MESA can offer a way out fromm this problem: at each evaluation od the likelihood, a new spectral analysis is performed by computing the PSD on $d_t-x_t$.
...
%To compute the posterior distribution for the parameters the knowledge of prior distributions and Likelihood for noise is required. A full description for noise can be obtained in Fourier Domain by the choice of the Whittle Likelihood \eqref{eq:Whittle}, that is completely determined by the knowledge of the PSD for noise. While traditional methods might be instable with respect to the hypothesis (such as Welch) or require a Large number of prior assumptions, MESA represents a stable and parsimonious way to characterize noise and compute posterior distributions.  
